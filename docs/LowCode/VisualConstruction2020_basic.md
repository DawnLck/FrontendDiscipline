...
- 如果面向的是**营销页面**或者**产品主页**的搭建，使用者通常是**非开发者**，整体的可视化解决方案就会更偏近 no-code。
- 如果面向的是**中后台页面搭建**，使用者是**开发者**，整体的可视化解决方案就会更偏近 low-code。
...
基础思路: 先调用一个模板作为基础骨架，然后通过控制组件的属性，调整组件的样式，使得大致的页面效果能够呈现出来，最后接入数据接口，进入构建和发布流程。

我们也可以从可视化搭建解决的痛点，或者说具有的能力维度，来分析这件事情。

![可视化搭建的能力维度](img/visualConstruction2020.23-51-46.png)。

为了达成这样的能力维度，其实需要很多层面的支撑，如**工具链**、**物料平台**、**工程套件**、**接口编排**等，下面展示了一张阿里妈妈的产品家族，大家可以略窥一二。
...
## 3. 低代码可视编辑器 Low-code Visual Editor
