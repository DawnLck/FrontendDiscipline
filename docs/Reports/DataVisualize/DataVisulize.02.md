...
> 数据可视化作为一个研究领域，衍生出了许多的可视化研究的课题。该篇作为前端可视化报告的第二篇，主要简述前端在实现数据可视化的时候，各类研究课题方向和相应的难点。每一个点后面都有大量的研究工作，笔者也没有办法在一篇的篇幅内，形成具有一定深度的技术挖掘，只能概览一下大体的几个发展路线，所以行文也会有些零散。
>
> 这里由于精力有限，主要调研的是 AntV 的相关工作，中间也穿插了一些很棒的数据可视化项目，欢迎点击参考文章部分的相关链接访问。
...
![Graph - Studio](https://img.alicdn.com/tfs/TB1laWQyVT7gK0jSZFpXXaTkpXa-600-600.gif)
...
![三维图可视化](https://img.zcool.cn/community/01d4835d6dd3f5a801211f9e894684.gif)
...
2. 百度度小满 - Buildings
   ...
   ![Duxiaoman](https://img.zcool.cn/community/01bb3b5d09a630a801213ec2d20856.gif)

   上图的这个作品对应的分类是 Buildings，目前流出的只有设计稿，猜测是百度的度小满团队在做，不知道完成度如何，大致推断应该是先利用 C4D 等工具先完成城市建模，再在此基础上完成的地理信息数据可视化平台。

3. 阿里 - DataV

   ![DataV - 杭州城市学区数据可视化](img/DataVisulize.02.16-06-45.png)
...
![动态柱状图](https://pic2.zhimg.com/v2-cf211053f01316269af4186be2631315_b.jpg)
...
[Tableau - Titanic: A Great Loss of Life](https://public.tableau.com/views/Titanic-Greatlossoflife/MainDashboard?:language=zh-Hans&:display_count=y&:origin=viz_share_link&:showVizHome=no)

![Tableau - Titanic: A Great Loss of Life](img/giphy.gif)
...
![LADV](img/DataVisulize.02.23-19-50.png)

从模型设计图看出，完成对设计稿上目标识别的主要核心部分是 [**Faster R-CNN（Region-CNN）**](https://pubmed.ncbi.nlm.nih.gov/27295650/)。

![LADV - Result](img/DataVisulize.02.23-25-51.png)

LADV 的结果样例如上，三组样例都是上方的是输入下方的是输出，可以看出还是保持了高度的还原的。

如果只是上面的，可能还不太够吸引到设计师去使用，所以 LADV 的团队做了另一个功能，接受一张风格图片作为另一个输入，从风格图提取色彩搭配，其中主色提取的部分采用了 DBSCAN 密度聚类算法。

![LADV - 色彩提取](img/DataVisulize.02.14-20-39.png)
...
![DeepInsight](img/DataVisulize.02.00-06-56.png)
...
![DeepInsight - 工作流](img/DataVisulize.02.00-19-19.png)
...
![DataV - 基础图表](img/DataVisulize.02.16-26-15.png)
![DataV - 地理信息图表](img/DataVisulize.02.16-26-41.png)
![DataV - 编辑器 - 模板](img/DataVisulize.02.16-25-54.png)
![DataV - 编辑器 - 界面](http://static-aliyun-doc.oss-cn-hangzhou.aliyuncs.com/assets/img/zh-CN/4894731061/p7715.gif)

## 9. 企业数据可视化解决方案

AntV 是蚂蚁金服全新一代数据可视化解决方案，致力于提供一套简单方便、专业可靠、不限可能的数据可视化最佳实践。从下图中可以看到整个 AntV 的架构，包含绘图引擎、通用、框架、中间件及应用。

![AntV - 企业级智能数据可视化解决方案](img/DataVisulize.02.23-54-17.png)

将上述的框架图的底层部分进行拆解，得到以下的一张流程图：

![AntV - 可视化语义系统](img/DataVisulize.02.23-59-26.png)

## 10. 其他方向

1. 多屏多端

   ![DataV - 多屏多端](img/DataVisulize.02.16-35-34.png)

2. 城市建模 -> 城市数字孪生 -> 智慧社区/智慧城市

   ![DataV - 三维城市数字孪生体系](img/DataVisulize.02.16-36-18.png)
